...
- **C4 Product Closure** — ▶ ТЕКУЩИЙ ЭТАП (`0.11.0-alpha.5`, Draft PR #22): device management (список устройств / переименование себя / admin-revoke; сервер `GET /v1/devices`, `POST /v1/devices/rename|revoke`), bootstrap нового устройства (replay всего stream с нуля, без JSON-импорта), экспорт диагностики (без секретов), disable/unlink явно отделён от удаления локальных данных. Детали — `docs/SYNC_C4_CLOSURE.md`. После мержа C4 Stage C закрыт → 0.12.x Today 2.0.

...
**0.12.0-alpha.2 (Material Usability):** обычная работа с уже созданным
материалом не требует возврата в Processing. В Inspector мысль/заметку можно
редактировать (текст) и переносить (Domain / Project / «Без контекста»);
библиотека «Мысли и заметки» получила поиск по тексту и фильтр «Без контекста».
Редактирование и перенос сохраняют тот же `id`, `kind` и `sourceInboxId`
(объект не пересоздаётся), идут только через Core-команду `updateKnowledge` и
не порождают Sync-операций (материал принадлежит Studio). После правки возврат
в разбор отказывается удалять изменённый материал — безопасность, а не дефект.
Проверено в Chromium: правка текста, перенос между проектами/доменами/без
контекста, поиск, фильтр и reload с тем же id. Next stage не начат.
...
**0.12.0-alpha.3 — Daily Reliability / Today Foundation:** закрыт связанный
с ежедневной работой срез надёжности перед Today 2.0. Ошибки startup больше
не заменяют данные demo-состоянием; Studio/Capture имеют экран восстановления,
предыдущий локальный снимок и сохранение оригинала перед явным восстановлением.
Команда сохраняет данные/operation log/Sync intent вместе (schema 7), затем
передаёт операции в durable outbox с безопасным повтором после сбоя.
Конкурирующие legacy Today/Inspector helpers отключены. Единый небольшой Today
отделяет выбранные задачи от сроков и показывает завершённое сегодня по
`completedAt`; действия идут через Core и C2 bridge. Проверены reload, отказ
записи/повтор, восстановление, второй браузерный клиент и Knowledge alpha.2.
Подробности и ограничения: `docs/DAILY_RELIABILITY.md`.

**0.12.0-alpha.4 — Knowledge Workspace / From Material to Action:** библиотека
материалов вынесена из узкого Inspector в основную рабочую область Studio.
Карточки показывают тип, фрагмент текста, контекст и число связанных действий;
доступны поиск по словам/контексту, фильтры и сортировка. Core создаёт Task со
ссылкой `sourceKnowledgeId` без преобразования/удаления исходной мысли или
заметки. Пользователь открывает результат в Inspector/Today и возвращается
к материалу-источнику. Защищены source links, session drafts и stale editor save.
Schema 8; полный Today 2.0 и Full Knowledge Sync не начаты. Подробности:
`docs/KNOWLEDGE_WORKSPACE.md`.

### 0.13.x — Today 2.0 (desktop + mobile) 🟠 ПЕРВЫЙ VERTICAL SLICE

**0.13.0-alpha.1 — Day Plan Foundation:** у задачи появился календарный день
планирования `plannedDay` (отдельно от срока `due` — срок остаётся дедлайном, а
не планом) и флаг `focus` (явный «Фокус дня», максимум 3 задачи, `FOCUS_LIMIT`).
Today показывает: Фокус дня, «Выбрано на сегодня», «Осталось с прошлых дней»
(незавершённое вчера не попадает в сегодняшний план автоматически — только
ручной перенос или возврат в backlog), «Срок наступил — ещё не выбрано» и
«Выполнено сегодня». Выбор/снятие/перенос/выполнение/возврат идут только через
Core (`updateTask`), `due` нигде не меняется, `completedAt` и C2-проекции
сохранили прежнюю семантику. Schema 9: легаси-задачи со `status: today|doing`
якорятся на день миграции. Задачи из Knowledge (`sourceKnowledgeId`)
планируются как обычные. Проверено focused regressions, baseline и browser
smoke (выбор → фокус → выполнение → reload → следующий день → перенос, due
неизменен). Оставлено на следующие 0.13.x: планирование на будущие дни,
оценка ёмкости, второй экран/мобильный Today.

Ориентир этапа: Сейчас / Focus-3 / план дня / ёмкость / переносы / выполнено; задачи после Processing Center.

**0.13.0-alpha.2 — Planning Ahead / Day Capacity:** день перестал быть
единственным горизонтом. «Сегодня» получил день как параметр (‹ ›, выбор даты,
возврат к сегодняшнему дню), и все группы считаются для выбранного дня; план на
будущий день ждёт в бэклоге этого дня, виден в разделе «Запланировано на
следующие дни» и сам становится планом дня, когда день наступает. Ёмкость дня
показывает число задач, сумму оценок и число задач без оценки (привычные 4 часа
— предупреждение, не запрет). Фокус остаётся решением на сегодня: перенос плана
на другой день снимает его, невозможный запрос отклоняется сообщением. В
Inspector появился раздел «План дня» (день, «Сегодня», «Завтра», «Убрать из
плана», фокус), а legacy-кнопка «Взять 3 задачи в Сегодня» удалена — она
выбирала первые три незавершённые задачи в обход `plannedDay`. Схема 10:
задача, запланированная на будущий день, больше не держит статус «сегодня».
Проверено focused regressions (`tests/today-forward-plan.mjs`), baseline и
браузерный smoke (день вперёд/назад, перенос плана, ёмкость, `due` неизменен).
Оставлено на следующие 0.13.x: недельный/месячный обзор, ёмкость как настройка
вместо константы, второй экран/мобильный Today.

Отдельно в этой же версии закрыт **Studio map-actionability pass** (карта как
рабочий инструмент, не как картинка): орб получил раздельные визуальные каналы
вместо серого шарика с радиусом-приоритетом, группы задач упаковываются с
зазором и честным счётчиком «+N задач», у задачи есть реальная зона попадания и
клавиатурная навигация, а один словарь статусов и дат (`js/ui/status-language.js`)
используют карта, инспектор, Today и Capture. Раньше этот проход упоминался как
отдельная развилка после closure Sync v1 — теперь он выполнен в рамках 0.13.x.

**Map honesty pass (0.13.x, в работе).** Проверка карты пробами и замерами нашла
дефекты, которые не видны в коде: домен с одним проектом ставил проект ровно в свой
центр (`orbit = 0`), из-за чего территории пересекались и задача попадала сразу в
две (`js/view_map.js`); в светлой теме подписи карты имели контраст 1.26–1.95:1,
то есть карта была практически без текста; вторичный текст — 4.36:1. Первые два
исправлены: раскладка домена считается от территорий детей (дом растёт под них,
дети уменьшаются только если домен переполнен), цвета карты читаются из темы
интерфейса, `--muted` светлой темы углублён. Проверено числами
(`output/map-prep-probe.mjs`), съёмкой и замерами
(`.agents/skills/atlas-visual-verification`), baseline зелёный. Остаток: слой
попадания, честность перетаскивания, перенос проекта через Core, счётчик материалов
домена, инструменты обзора — см. `docs/MAP_IMPROVEMENTS.md` и `docs/WORK_LEDGER.md`.

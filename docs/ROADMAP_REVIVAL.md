...
- **C3 Conflicts & Recovery** — ▶ КОД ГОТОВ (`0.11.0-alpha.4`, **Draft PR #21 — ветка `feat/sync-c3-conflicts-alpha4`, НЕ замержен в `revival-preparation`**): человеческие конфликты вместо «detect+refuse» — классификация (base_version / deleted_race / delete_restore_race / linked_result_delete), действия в панели Sync (оставить локальную / принять удалённую / сохранить обе / восстановить и применить / удалить), resolution durable; tombstones удалённых записей; синхронизация удаления/восстановления Inbox (W2); rename Domain/Project обновляет проекции на телефоне (W3); failed-ретраи после долгого офлайна; terminal `rejected` для server-отклонённых операций. Детали — `docs/SYNC_C3_CONFLICTS.md`.
- **C4 Product Closure** — ▶ ТЕКУЩИЙ ЭТАП (`0.11.0-alpha.5`, **Draft PR #22 — ветка `feat/sync-c4-closure-alpha5`, stacked на C3, НЕ в `revival-preparation`**): device management (список устройств / переименование себя / admin-revoke; сервер `GET /v1/devices`, `POST /v1/devices/rename|revoke`), bootstrap нового устройства (replay всего stream с нуля, без JSON-импорта), экспорт диагностики (без секретов), disable/unlink явно отделён от удаления локальных данных. Детали — `docs/SYNC_C4_CLOSURE.md`. После review и мержа C4 Stage C закрыт → 0.12.x Today 2.0.
- **Closure-gates (НЕ закрыты):** VDS-деплой + HTTPS, физический прогон телефон ↔ ПК — требуют доступов пользователя; браузерные smoke в этой среде требуют escalated-запуска.

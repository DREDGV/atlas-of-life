...
- Сохранять существующие инварианты (если код эволюционировал — правило формулируется по текущему фактическому состоянию):
  - `rawText` immutable;
  - Inbox `resultRef` ↔ Task/Knowledge `sourceInboxId` (двусторонняя связь);
  - безопасные routing/revert (routed-записи залочены по типу/статусу; revert не удаляет изменённый Task/Knowledge результат);
...
- В начале задачи определить пользовательский результат и границы работы. Для запроса на анализ/план не начинать реализацию. Для запроса на реализацию самостоятельно пройти согласованный сценарий до результата; внутренний план не требует отдельного согласования.
- Читать актуальные инструкции один раз за задачу; повторять чтение при смене checkout или изменениях файлов. Большие документы искать по нужным разделам. Исторические handoff и старые Master Roadmap — справочные снимки, не источник текущего статуса.
- Использовать только skills, подходящие к действию: `atlas-development` для реализации, `atlas-ui-review` для интерфейса; `frontend-design` — для нового/существенно изменяемого UI с сохранением дизайна Atlas. Не подключать дополнительные workflows только по совпадению ключевых слов.
- Перед исправлением проследить реальный вход в код, включая подключённые `addons/`. Подтверждённый дефект, риск по коду и продуктовая гипотеза должны различаться в отчёте.
- В Windows читать текст с явным UTF-8. Для многострочных и кириллических правок предпочитать `apply_patch`; не передавать исходники через PowerShell pipeline в Python без явной проверки кодировки. Не передавать JS-файлы PowerShell для разбора.
- Browser smoke выполнять в отдельном профиле с тестовыми данными и loopback relay. Доступность Playwright/браузера проверять по текущей установке; не копировать устаревший абсолютный путь к Chromium. QA-артефакты — `output/playwright/`.
- При изменении persisted lifecycle проверять успешное сохранение, reload и подходящий отказ/возврат. При изменении Sync проверять второй изолированный клиент. Зелёный синтаксис/текстовые assertions не заменяют пользовательский сценарий.
- Делегирование применять, когда оно разрешено текущими инструкциями и есть независимая задача. Количество агентов и максимальная глубина reasoning сами по себе не являются целью.

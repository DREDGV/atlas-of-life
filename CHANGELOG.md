## Atlas_of_life_v0.8.6.0 (02.10.2025)

### ⚡ Система кэширования иерархии
- **Новый модуль кэширования** - `js/hierarchy/cache.js` с TTL и автоматической очисткой
- **Кэшированные функции** - `getCachedParent()`, `getCachedChildren()`, `getCachedAncestors()`
- **Автоматическая инвалидация** кэша при изменениях иерархии
- **Статистика кэша** в инспекторе с показателями эффективности
- **Управление кэшем** - очистка и мониторинг производительности

### 🎨 Улучшения интерфейса
- **Статистика кэша** в инспекторе домена с сеткой показателей
- **Кнопка очистки кэша** с подтверждением действия
- **Мониторинг производительности** иерархических запросов

### 🔧 Технические улучшения
- **Интеграция кэширования** во все операции attach/detach/move
- **Оптимизация поиска** родителей, детей и предков
- **Умная инвалидация** - только затронутые объекты
- **Настраиваемые параметры** - размер кэша, TTL, лимиты

---

## Atlas_of_life_v0.8.5.0 (02.10.2025)

### ✨ Система истории связей
- **Добавлено поле `history`** во все объекты для отслеживания изменений связей
- **Функции работы с историей**: `addHierarchyHistory()`, `getHierarchyHistory()`, `clearHierarchyHistory()`
- **Автоматическая запись истории** при всех операциях attach/detach/move
- **Просмотр истории связей** в инспекторе для всех типов объектов
- **Возможность отката изменений** с кнопками 🔄 для каждой записи истории
- **Миграция существующих объектов** для добавления поля history

### 🎨 Улучшения интерфейса
- **Стилизованная история связей** с иконками действий и временными метками
- **Выделение последнего изменения** в истории
- **Интерактивные кнопки отката** с подтверждением действия
- **Адаптивный дизайн** истории связей

### 🔧 Технические улучшения
- **Интеграция с системой иерархии v3** для автоматического логирования
- **Ограничение истории** до 10 последних записей на объект
- **Детальная информация** в записях истории (родители, типы, названия)
- **Обработка ошибок** при откате изменений

---

## Atlas_of_life_v0.8.4.0 (29.09.2025)

### ✅ Полная интеграция иерархической системы v3
- 🚀 **Новый модуль иерархии** - `js/hierarchy/index.js` с матрицей допустимых связей и валидацией
- 🎯 **Единые API** - `attachChild`, `detachChild`, `moveChild` с проверкой циклов и блокировок
- 🔧 **Интеграция DnD** - все функции `confirm*Move/Detach` переведены на новый API
- ⚡ **Интеграция инспектора** - кнопки действий используют единые API с понятными ошибками
- 🛡️ **Журнал изменений** - история связей в инспекторе (последние 5 записей)
- 🔄 **Автоаудит при загрузке** - проверка целостности и автоисправление простых ошибок
- 🔒 **Система блокировок** - поддержка `locks.move` и `locks.hierarchy` для объектов

### 🔧 Технические улучшения
- **js/hierarchy/index.js**: Матрица разрешений, индексы, attach/detach/move, валидация циклов
- **js/state.js**: Обёртки API, журнал изменений, совместимость со старыми функциями
- **js/view_map.js**: Перевод всех confirm* функций на новый API с обработкой ошибок
- **js/inspector.js**: Интеграция с API, история связей, улучшенные сообщения об ошибках
- **js/app.js**: Аудит иерархии при загрузке с автоисправлением

### 🎯 Результат
- ✅ Надёжная валидация связей с предотвращением циклов
- ✅ Единообразные API для всех операций с иерархией
- ✅ Понятные сообщения об ошибках для пользователя
- ✅ История изменений связей в инспекторе
- ✅ Автоматическая проверка целостности при загрузке
- ✅ Поддержка блокировок для защиты от случайных изменений

---

## Atlas_of_life_v0.8.3.0 (29.09.2025)
...
- fix: add forced hierarchy auto-fix in app.js (83a654c)
- fix: add automatic hierarchy validation fixes (d2ea55f)
- fix: add migration to clean up invalid parentId references (ef0d314)
- fix: improve hierarchy validation and logging (6b483a2)
- fix: remove duplicate renderHierarchyHistory function declaration (065c406)
- feat: implement hierarchy history system v0.8.5.0 (2572153)
- docs: ╨╛╨▒╨╜╨╛╨▓╨╕╤В╤М roadmap - ╨╛╤В╨╝╨╡╤В╨╕╤В╤М ╨╖╨░╨▓╨╡╤А╤И╨╡╨╜╨╕╨╡ ╤Н╤В╨░╨┐╨╛╨▓ 3-5 ╨╕╨╡╤А╨░╤А╤Е╨╕╤З╨╡╤Б╨║╨╛╨╣ ╤Б╨╕╤Б╤В╨╡╨╝╤Л (42e9262)
- feat: ╨┐╨╛╨╗╨╜╨░╤П ╨╕╨╜╤В╨╡╨│╤А╨░╤Ж╨╕╤П ╨╕╨╡╤А╨░╤А╤Е╨╕╤З╨╡╤Б╨║╨╛╨╣ ╤Б╨╕╤Б╤В╨╡╨╝╤Л v3 (1d0e8e5)
- feat: v0.8.3.0 - simplify theme system and fix color inconsistencies (a83edfd)
...
- fix: update checklist coordinates in layoutMap to fix hit detection mismatch (3af3e06)

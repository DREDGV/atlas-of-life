...
История версий (релизы) + дорожная карта (планы/идеи). Актуальная версия UI берётся из `js/version.js`.

## 0.13.0-alpha.2 — Today 2.0 / Planning Ahead + Day Capacity

- **Планирование вперёд:** день задачи больше не привязан к «сегодня». Задача, запланированная на будущий день, ждёт в бэклоге этого дня и видна в «Сегодня» в разделе «Запланировано на следующие дни», а в свой день становится планом дня сама. Срок `due` остаётся крайним сроком и никогда не двигается планированием.
- **День как параметр «Сегодня»:** переключатель ‹ ›, выбор даты и возврат к сегодняшнему дню; группы (фокус, выбранное, остаток, сроки, выполненное) считаются для выбранного дня. Выбранный день — состояние сессии: при входе в «Сегодня» экран открывается на сегодняшнем дне.
- **Ёмкость дня:** строка плана показывает число задач, сумму оценок и число задач без оценки; превышение привычных 4 часов — предупреждение, а не запрет. Оценка не выдумывается.
- **Фокус дня остаётся решением на сегодня:** в фокус можно взять только задачу, выбранную на сегодня, и не больше трёх; перенос плана на другой день снимает фокус, а невозможный запрос отклоняется с сообщением вместо тихого игнорирования.
- **Статус следует за днём:** `plannedDay` в будущем → бэклог (план), сегодня или раньше → работа (план дня или остаток, ждущий переноса). Явно указанный статус по-прежнему главнее и определяет только состояние работы, а не день.
- **Inspector:** раздел «План дня» — выбор дня, «Сегодня», «Завтра», «Убрать из плана» и фокус дня. Legacy-кнопка «Взять 3 задачи в Сегодня» удалена: она выбирала первые три незавершённые задачи в обход `plannedDay`.
- **Карта:** орб перестал быть серым шариком — каналы разделены (заливка — статус, цвет — срочность срока, кольцо — сегодня/работа/просрочка, крестик — фокус, дуга — давность, размер — важность и оценка), группы задач упаковываются с зазором и честным счётчиком «+N задач», подписи выбираются по расстоянию до соседа, а легенда объясняет словарь орба.
- **Доступность карты:** у задачи есть реальная зона попадания (не меньше 26 CSS-пикселей), canvas фокусируем и управляется стрелками, Tab/Shift+Tab, Home/End, Enter/Space и Escape; выбранная с клавиатуры задача обведена кольцом, её название и срок объявляются через live-region; клик по переполненной группе приближает камеру к её задачам.
- **Единый язык статусов и дат** (`js/ui/status-language.js`) для карты, инспектора, «Сегодня» и Capture: ISO-даты и сырые `backlog`/`doing` больше не показываются, просрочка читается как «Срок истёк: 9 сен · 2 дня назад».
- Схема 10: задача, запланированная на будущий день, перестаёт держать статус «сегодня» (раньше она была невидима с обеих сторон плана); незавершённые задачи прошлых дней сохраняют статус работы — это остаток.
- Проверено: `tools/verify-baseline.ps1` (все suite), `tests/today-forward-plan.mjs`, `tests/map-access.mjs`, `tools/smoke-today-plan.mjs` в реальном Chromium.

## 0.13.0-alpha.1 — Today 2.0 / Day Plan Foundation

- `plannedDay` — календарный день планирования, отдельно от срока `due`; флаг `focus` (Фокус дня, максимум 3 задачи).
- «Сегодня» показывает Фокус дня, «Выбрано на сегодня», «Осталось с прошлых дней», «Срок наступил — ещё не выбрано» и «Выполнено сегодня».
- Выбор, снятие, перенос, выполнение и возврат идут только через Core (`updateTask`); `due` нигде не меняется.
- Схема 9: легаси-задачи со `status: today|doing` якорятся на день миграции.

## 0.12.0-alpha.4 — Knowledge Workspace / From Material to Action

- Полноценная библиотека в основной области Studio: карточки, поиск по словам, типы, контексты, сортировка и состояние «Без контекста»; поддержаны тёмная/светлая темы и компактное окно.
- Из мысли/заметки можно создать связанную задачу в её контексте и выбрать её на сегодня. Материал и исходный Inbox не пересоздаются; задача открывает свой материал-источник.
- Core владеет `Task.sourceKnowledgeId`; связанные действия вычисляются из Tasks. Возврат материала блокируется, пока существуют связанные задачи. Схема 8 защищает новые связи от старых писателей.
- Несохранённые правки материала переживают переключение карточек в текущей сессии; устаревшая версия редактора не перезаписывает изменённый материал. Hotkeys карты не перехватывают ввод в формах.

## 0.12.0-alpha.3 — Daily Reliability / Today Foundation

- Ошибка чтения, повреждённые данные и неподдерживаемая схема больше не включают demo fallback. Studio/Capture показывают восстановление и не запускают Sync.
- Один предыдущий локальный снимок перед записью; скачивание оригинала, явное восстановление/импорт через Core с сохранением данных до замены. Панель доступна через «Ещё → Резервные данные».
- Команда атомарно сохраняет данные, журнал и намерение отправки Sync. Outbox можно повторно заполнить после сбоя; операции сохраняют прежние IDs и v1-контракт.
- Legacy «Сегодня+», Inspector helpers и устаревший ICS exporter отключены. Удалён неиспользуемый старый domain-menu с прямым каскадным изменением Tasks.
- Единый Today: выбранные/в работе, наступившие сроки отдельно, выполненное сегодня и возврат. Завершение через Core сохраняет `completedAt`, переживает reload и обновляет C2-проекцию на Capture.
- Схема 7; focused recovery/Sync regression и реальные двухклиентные browser smoke. Today 2.0, календарь и Full Sync не начаты.
...
- feat(today): plan ahead — the day becomes a parameter (0.13.0-alpha.2) (a717cca)
- feat(today): planning ahead — day as a view parameter, day capacity, forward plans (0.13.0-alpha.2)
- feat(map): inform the task orb and make it reachable — packing, keyboard navigation, hit target (0.13.0-alpha.2)

...
- **Progressive disclosure**: сначала главный вопрос «Что это?» (Задача / Мысль / Заметка); для Задачи раскрывается «Куда?» (Domain/Project + «Создать задачу»), приоритет и срок спрятаны за «▸ Дополнительно»; для Мысли/Заметки — одно действие «Считать разобранной» (itemType + processed, без Task)
- **Последовательный разбор**: одна активная/раскрытая карточка, остальные компактные; после обработки запись уходит из «К разбору» и следующая становится основной
...
- **Визуальная чистка**: одна очевидная primary-кнопка на шаг, вторичные действия визуально слабее, «Отбросить» — отдельное действие, «Удалить» — вторичное; форма маршрутизации — последовательность решений, а не таблица параметров

#### Техническое
- `InboxItem.domainHintId` — Capture-подсказка домена (hint, не финальный route); нормализация в `addInboxLines`
- Все persisted-изменения по-прежнему через Core-команды (`inbox.update`): авто-reviewed, processed для Thought/Note, discarded
- UI-черновики (routing draft, capture hints) остаются ephemeral и не пишутся в storage
- Focused regression tests: `tests/processing-b2.mjs`
